...
- (CGRect)frameOfEnterButton{
    ...
    return CGRectMake(self.view.frame.size.width / 2 - size.width / 2, self.view.frame.size.height*0.855 , size.width, size.height);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    //单项滚动
    if (scrollView.contentOffset.x < self.view.frame.size.width*self.pageControl.currentPage) {
        [scrollView setContentOffset:CGPointMake(self.view.frame.size.width*self.pageControl.currentPage, scrollView.contentOffset.y) animated:NO];
    }else{
